...
table(pheno$codition)

dds = DESeqDataSetFromMatrix(countData = exp.count , colData = pheno , design = ~condition)
dds.run = DESeq(dds)
...
contrast1 = results(dds.run, contrast = c("condition", "WT mice control diet",
...
top50_DEGS = row.names(exp.degs)[1:50]
exp50_DEGS = exp.degs[top50_DEGS,]

column_ha = HeatmapAnnotation(sample.type = pheno$condition)
Heatmap(exp50_DEGS,name = 'Exp', row_names_gp= gpar(fontsize=3) , column_names_gp = gpar(fontsize=10)
        , top_annotation = column_ha)


############### 2D PCA ############3
expression_t= t(exp50_DEGS)
...
# 1. boxplot
boxplot(exp50_DEGS,main = "processed genes Box Plot",col=seq(1:12))

# 2. histogram
cols=colnames(exp50_DEGS)
